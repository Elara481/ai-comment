# Supabase 数据库建表与配置指南 / Supabase データベーステーブル作成・設定ガイド

<p align="center">
  <a href="#supabase-zh">🇨🇳 中文</a> ／ <a href="#supabase-ja">🇯🇵 日本語</a>
</p>
...
1. 左側の **Project Settings** → **API** をクリックする。
2. **Project URL** と **anon public key** を見つけてコピーしておく。
...
1. [Render Dashboard](https://dashboard.render.com/) を開き、`ai-comment` Web Service に入る。
2. 左側の **Environment** をクリックする。
3. 以下の 2 つの環境変数を追加する：
   - `SUPABASE_URL`：あなたの Project URL
   - `SUPABASE_KEY`：あなたの anon public key
4. 保存後、Render が自動で再起動し、社畜広場のデータが永続化されます！
